...
const statusBadge: Record<StockStatus, { bg: string; text: string; label: string }> = {
  'low-stock':    { bg: 'bg-[#FEF3C7]', text: 'text-[#B45309]', label: 'LOW STOCK' },
  'out-of-stock': { bg: 'bg-[#FFE4E6]', text: 'text-[#BE123C]', label: 'OUT OF STOCK' },
  'in-stock':     { bg: 'bg-[#D1FAE5]', text: 'text-[#047857]', label: 'IN STOCK' },
}
              ...
              {notifications.map(item => (
                ...
                >
                  <div className="min-w-0">
                    <p className="font-[Archivo] text-sm font-semibold text-[#171a1f] truncate">{item.title}</p>
                    <p className="font-[Archivo] text-xs text-[#565e6c] mt-0.5">{item.subtitle}</p>
                  </div>
                  {item.kind === 'stock' ? (
